...
func (u *uvInitializer) upgrade(uvPath string) error {
	out, err := exec.Command(uvPath, "self", "update").CombinedOutput()
	if err == nil {
		return nil
	}

	msg := strings.TrimSpace(string(out))
	// uv 通过 pip/brew 等包管理器安装时，不支持 self update
	if strings.Contains(msg, "Self-update is only available") {
		pterm.Warning.Println("当前 uv 非脚本安装，无法执行 `uv self update`，已跳过自动升级。")
		pterm.Warning.Println("请使用你的安装方式升级：`brew upgrade uv` 或 `pip install -U uv`。")
		return nil
	}
...
// configureMirror 当 OPEN_CODE_PROXY_URL 不为空时，配置 uv 国内镜像源
func (u *uvInitializer) configureMirror() {
	proxyURL := os.Getenv("OPEN_CODE_PROXY_URL")
	if proxyURL == "" {
		return
	}
	...
	// 镜像源配置内容
	mirrorConfig := `# 由 open-code --init 自动生成
